...
function AutoUpdate(data)
    if tonumber(data) > tonumber(rver) then
        ...
        DownloadFileAsync("https://raw.githubusercontent.com/Eric904P/GOS/master/HK_Ryze.lua", SCRIPT_PATH .. "IcyRyze.lua", function() PrintChat("Update Complete, please 2x F6!") return end)
    else
        PrintChat("No updates found, IcyRyze Loaded!")
    end
end

GetWebResultAsync("https://raw.githubusercontent.com/Eric904P/GOS/master/HK_Ryze.version", AutoUpdate)
...
Menu.misc:Boolean("astack", "Auto stack tear", true)
Menu.misc:Slider("astackmana", "Min mana for auto stacking", 80, 0, 100, 1)
...
function AutoStack()
  ...
  if Menu.misc.astack:Value() then
    if IsObjectAlive(myHero) then
    if (myHero.mana/myHero.maxMana >= Menu.misc.astackmana:Value() /100) then
